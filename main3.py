...
async def my_event_handler(event):
    try:
        ...
        logger.info(f"NewMessage: chat={event.chat_id}, id={event.message.id}, grouped_id={event.message.grouped_id}, media={bool(event.message.media)}, media_type={media_type}, moderation={moderation_active}")
    except Exception:
        pass
    if event.message.grouped_id:
        # Ждём альбом; если альбом обработается, это сообщение будет отмечено и мы его пропустим
    ...
    if destination_channel_id is None:
        logger.info(f"Пропуск сообщения {event.message.id} из канала {event.chat_id}: нет соответствия в channel_mapping")
        return
    ...
    try:
        ...
        logger.info(f"Сообщение переслано: {original_text} из канала {event.chat_id} в канал {destination_channel_id}")
    except Exception as e:
        logger.error(f"Ошибка при отправке сообщения {event.message.id} из {event.chat_id}: {str(e)}")
...
@client.on(events.Album(chats=list(channels)))
async def album_event_handler(event):
    grouped_media = event.messages
    logger.info(f"Album: chat={event.chat_id}, size={len(grouped_media)}; ids={[m.id for m in grouped_media]}")
    updated_texts = []
    media_list = []
    ...
    destination_channel_id = channel_mapping.get(event.chat_id)
    if destination_channel_id is None:
        logger.info(f"Пропуск альбома из канала {event.chat_id}: нет соответствия в channel_mapping")
        return

    try:
        await client.send_file(destination_channel_id, media_list, caption=updated_caption)
        # Помечаем все сообщения альбома как обработанные, чтобы одиночный хендлер их не дублировал
        for m in grouped_media:
            processed_message_ids.add(m.id)
        logger.info(f"Альбом переслан из {event.chat_id} в {destination_channel_id}")
    except Exception as e:
        logger.error(f"Ошибка при отправке альбома из {event.chat_id} в {destination_channel_id}: {str(e)}")
                    ...
                    if msg.media:
                        if isinstance(msg.media, MessageMediaWebPage):
                            ...
                            if len(updated_text) > 1024:
                                updated_text = updated_text[:1021] + "..."
